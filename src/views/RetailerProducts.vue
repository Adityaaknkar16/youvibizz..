<template>
  <div class="topbar">
    ...
    <span class="topbar-title">Product Inventory</span>
  ...
  <div class="sheet">
    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:16px;">
      <h3 style="font-size:17px;font-weight:800;color:var(--navy);">My Products</h3>
      <RouterLink to="/retailer/products/add" class="btn btn-teal btn-xs" style="text-decoration:none;">+ Add Product</RouterLink>
    </div>
    ...
    <div v-if="loading" style="text-align:center;padding:20px;color:var(--muted);">Loading products...</div>
    <div v-else-if="error" style="text-align:center;padding:20px;color:var(--danger);">{{ error }}</div>

    <div v-for="product in filteredProducts" :key="product.id" class="product-card">
      <div class="product-card-img">
        <img :src="product.image" :alt="product.name" @error="$event.target.src='https://images.unsplash.com/photo-1504917595217-d4dc5ebe6122?w=400'">
        <div class="product-status">
          <span class="pill" :class="statusPillClass(product.status)">{{ product.status }}</span>
        </div>
      </div>
      <div class="product-body">
        <div class="product-name">{{ product.name }}</div>
        <div class="product-cat">{{ product.category }} · {{ product.spec }}</div>
        <div class="product-footer">
          <div class="product-actions">
            <button class="icon-btn" title="Edit" @click="editProduct(product)">
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M11 4H4a2 2 0 00-2 2v14a2 2 0 002 2h14a2 2 0 002-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 013 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>
            </button>
            <button class="icon-btn" title="Delete" style="color:var(--danger);border-color:var(--danger);" @click="deleteProduct(product.id)">
    ...
    <div v-if="!loading && filteredProducts.length === 0" style="text-align:center;padding:30px;color:var(--muted);">
      No products found.
    </div>
...
const products = ref([
  { id: 1, name: 'Gurjan Face Commercial Ply', category: 'Commercial Plywood', spec: '18mm', status: 'Active',  image: 'https://images.unsplash.com/photo-1504917595217-d4dc5ebe6122?w=400' },
  { id: 2, name: 'Marine Grade Ply',           category: 'Marine Plywood',    spec: '12mm', status: 'Active',  image: 'https://images.unsplash.com/photo-1600607687920-4e2a09cf159d?w=400' },
  { id: 3, name: 'HDF Boards 6mm',             category: 'HDF',               spec: '6mm Standard', status: 'Pending', image: 'https://images.unsplash.com/photo-1600607687939-ce8a6c25118c?w=400' },
  { id: 4, name: 'Decorative Laminates',       category: 'Laminates',         spec: 'Premium Range', status: 'Active', image: 'https://images.unsplash.com/photo-1595428774223-ef52624120d2?w=400' },
])
...
function statusPillClass(status) {
  return { Active: 'pill-green', Pending: 'pill-yellow', Inactive: 'pill-gray' }[status] || 'pill-gray'
}
...
function editProduct(product) {
  router.push(`/retailer/products?edit=${product.id}`)
}
